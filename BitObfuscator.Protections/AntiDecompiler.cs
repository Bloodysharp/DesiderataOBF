...
{
    public class AntiDecompiler : IObfuscationProtection
    {
        public void Apply(ObfuscationContext context)
        {
            foreach (var method in context.Module.Types.SelectMany(t => t.Methods).Where(m => m.HasBody))
            {
                ...
                proc.InsertBefore(method.Body.Instructions[0], proc.Create(OpCodes.Sizeof, context.Module.TypeSystem.Int64));
            }
        }
    }
}
